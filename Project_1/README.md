...
The subway stations were ranked using a weighted scoring system that accounted for "daily volume", "women population", "median income" and "In Tech Zone". As a starting point, the weights were set to a ratio of 3:2:2:1 for the four factors, respectively. We would like to tweek these parameters further with the client as the next step. Unfortunately, we didn't have enough time to finalize the visualizaiton of the recommended stations on a city map. I would like to find a clean way of doing that. Perhaps a follow up post!
